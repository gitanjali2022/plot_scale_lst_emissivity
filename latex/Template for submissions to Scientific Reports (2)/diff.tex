...
%DIF ADD LST_revis_round2.tex   Thu Apr 28 09:06:37 2022
...
\DIFdel{. This procedure is repeated using a predefined range of }\DIFdelend \DIFaddbegin \DIFadd{(}\url{https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.linregress.html}\DIFadd{). The $T_{s}$ is estimated by solving Eq. (\ref{eq_Tleq}) and Eq. (\ref{eq_Tseq}) using measured longwave radiation and prescribed }\DIFaddend $\epsilon$\DIFdelbegin \DIFdel{from 0.4 until 0.998 with a }\DIFdelend \DIFaddbegin \DIFadd{, starting with the maximum possible value for a grey body, 0.99. $\epsilon$ is progressively reduced with }\DIFaddend step size of 0.002 \DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{From all the values of $\epsilon$ that resulted in strong correlation }\DIFdelend \DIFaddbegin \DIFadd{until we reach a minimum RMSE (with $R^{2}>0.5$) for a linear relationship }\DIFaddend between $H$ and \DIFdelbegin \DIFdel{$T_{s}-T_{a}$ (}\DIFdelend \DIFaddbegin \DIFadd{$\Delta T$. Only those months }\DIFaddend with $R^{2}>0.5$ \DIFdelbegin \DIFdel{), the value that achieved the smallest RMSE was chosen as the suitable $\epsilon$ for the given month}\DIFdelend \DIFaddbegin \DIFadd{between $H$ and $\Delta T$ is considered for $\epsilon_{plot}$ estimation}\DIFaddend . An illustration plot for RMSE and $\epsilon$ is shown in SI \DIFdelbegin \DIFdel{Figure5. The $\epsilon$ is obtained for each month using both, }\DIFdelend \DIFaddbegin \DIFadd{Figure 5. The monthly $\epsilon_{plot}$ is obtained using }\DIFaddend long (Eq. (\ref{eq_Tleq})) and short equation (Eq.(\ref{eq_Tseq})) and termed as $\epsilon_{leq}$ and $\epsilon_{seq}$ respectively, as shown in Fig. \ref{fig:flow_chart}b. For sites \DIFdelbegin \DIFdel{like HS and LF resulting in a }\DIFdelend \DIFaddbegin \DIFadd{having }\DIFaddend high value of intercept \DIFdelbegin \DIFdel{, we hypothesized an underestimation of $R_{lup}$ and corrected it by adding $6-8\%$ of the observed day time }\DIFdelend \DIFaddbegin \DIFadd{like HS and LF (Table \ref{table:eps_comp}) the measured $R_{lup}$ is corrected by adding 6-8\% of }\DIFaddend observed $R_{lup}$ \DIFdelbegin \DIFdel{to the measured data before closing the energy balance }\DIFdelend \DIFaddbegin \DIFadd{and energy balance is closed }\DIFaddend using Bowen ratio closure \DIFdelbegin \DIFdel{, and estimating monthly plot-scale $\epsilon$.
}\DIFdelend \DIFaddbegin \DIFadd{and $\epsilon_{plot}$ is estimated (Fig. \ref{fig:mxc_dis}).
}

\DIFaddend Recently, another approach for plot-scale $\epsilon$ estimation using Eq. (\ref{eq_Rlup}) was discussed by Maes et.al (2019)\cite{maes2019potential}. For each month, the observed $R_{lup}$,  $R_{ldwn}$, $T_{a}$  are filtered for non rainy days using the filtering criteria ($-2 < H <2$ , and $\alpha < 0.4$). The $\epsilon$ is estimated by substituting $T_{s} = T_{a}$ in Eq. (\ref{eq_Rlup}) as shown in Eq. (\ref{eq_e_Maes}). The monthly $\epsilon$  was \DIFdelbegin \DIFdel{represented }\DIFdelend \DIFaddbegin \DIFadd{obtained }\DIFaddend as the median of $\epsilon$ obtained by substituting filtered data in Eq. (\ref{eq_e_Maes}) \DIFaddbegin \DIFadd{(red stars in Fig. \ref{fig:HDT})}\DIFaddend . 
\begin{equation}\label{eq_e_Maes}
\epsilon = \frac{R_{ldwn} - R_{lup}}{R_{ldwn} - T_{a}^{4} \sigma}
\end{equation}

\textbf{LST comparison}: MODIS LSTs are a global reference for LST and used world-wide, also in conjuction with plot-scale flux measurements.Plot-scale LST based on optimum $\epsilon$ for each month using a linear regression with and without intercept $H(\Delta T)$ is compared to the MODIS daily LST (without \DIFdelbegin \DIFdel{correcting }\DIFdelend \DIFaddbegin \DIFadd{correction }\DIFaddend for the viewing angle). For each month the $T_{s}$ estimates using plot-scale $\epsilon$ and MODIS $\epsilon$ corresponding to the exact TERRA day-time of pass was obtained (30 minute tower data was upsampled to each minute using linear interpolation). TERRA (satellite) overpasses at local solar time between 10:30 am to 12 pm in ascending mode \cite{guillevic2017land}. Plot-scale LST at local satellite overpass ( $T_{leq}$ and $T_{seq}$) was obtained using measured longwave radiation that was interpolated to match the exact time of overpass and corresponding monthly plot-scale $\epsilon$. Plot-scale daily LST is compared to MODIS LST in terms of mean, bias, RMSE and $R^2$ using a robust linear regression model (scipy stat model\DIFdelbegin \DIFdel{,}%DIFDELCMD < \url{https://github.com/scipy/scipy/blob/v1.7.1/scipy/stats/_stats_mstats_common.py} 
%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend as shown in Fig. \ref{fig:flow_chart}a. The goodness of fit between plot-scale and landscape-scale LST was determined by looking at $R^2$ (Fig. \ref{fig:flow_chart}b). The bias is estimated as the mean of deviation between daily MODIS LST and ground-based $T_{s}$ ($T_{leq, seq} - T_{MODIS}$). In \DIFdelbegin \DIFdel{past ground based radiometric measurement }\DIFdelend \DIFaddbegin \DIFadd{the past, ground-based radiometric measurements }\DIFaddend have been used for validation of \DIFaddbegin \DIFadd{the }\DIFaddend MODIS LST product \cite{wang2008validating}\DIFdelbegin \DIFdel{whereas the we comapred }\DIFdelend \DIFaddbegin \DIFadd{, whereas here we compared }\DIFaddend the plot-scale LST with MODIS LST as \DIFaddbegin \DIFadd{a }\DIFaddend sanity check. See SI Table 1 for data sources and acronyms.
