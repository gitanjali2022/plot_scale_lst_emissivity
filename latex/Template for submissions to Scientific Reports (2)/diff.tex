...
%DIF DEL LST_revis_round1.tex   Thu Apr 14 10:03:11 2022
%DIF ADD LST_revis_round2.tex   Wed May  4 16:59:02 2022
...
\graphicspath{{./Plots/}}
%DIF 34c34
%DIF < \title{Accounting for downwelling longwave radiation reduces the contribution of emissivity to uncertainty in surface temperature estimates from flux tower data}
%DIF -------
\title{Downwelling longwave radiation and sensible heat flux observations are critical for surface temperature and emissivity estimation from flux tower data} %DIF > 
%DIF -------
...
%DIF 41a40
\author[1]{Kaniska Mallick} %DIF > 
%DIF -------
\author[1]{Ivonne Trebs}
\author[1]{Mauro Sulis}
%DIF 43-44d43
%DIF < \author[1]{Kaniska Mallick }
%DIF < \affil[1]{Luxembourg Institute of Science and Technology, ERIN, Belvaux, L-4422, Luxembourg}
%DIF -------

%DIF 46a44-45
\affil[1]{Environmental Sensing and Modelling unit (ENVISION), Environmental Research and Innovation department (ERIN), Luxembourg Institute of Science and Technology (LIST), Belvaux, Luxembourg} %DIF > 
 %DIF > 
%DIF -------
...
The emitted and \DIFdelbegin \DIFdel{down-welling }\DIFdelend \DIFaddbegin \DIFadd{downwelling }\DIFaddend longwave radiance are measured at given angle within its instantaneous field of view (fov) by a downward facing sensor relatively close to the surface (a few meters for an eddy covariance tower). The radiation received by a pyrgeometer or infrared sensor is a combination of the radiation emitted ($R_{lem}$) and reflected ($R_{lref}$) by the surfaces in its fov as shown in Eq. (\ref{eq_Rlup})\DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{:
}\DIFaddend \begin{equation}\label{eq_Rlup}
R_{lup} = R_{lem} + R_{lref}
\end{equation}
Substitution of Eq. (\ref{eq_Rlem}) into Eq. (\ref{eq_Rlup}) and replacing  $\alpha$ as $1 - \epsilon$, $R_{lup}$ becomes a function of emissivity, surface temperature and \DIFdelbegin \DIFdel{down-welling }\DIFdelend \DIFaddbegin \DIFadd{downwelling }\DIFaddend longwave radiation:
...
\end{equation} 
Note that the above derivation is actually flawed, as the second term of Eq. (\ref{eq_leq}) was omitted arguing that $\epsilon\approx 1$, and yet $\epsilon$ was retained in the first part of the equation. Nevertheless, even with the availability of \DIFdelbegin \DIFdel{down-welling }\DIFdelend \DIFaddbegin \DIFadd{downwelling }\DIFaddend longwave measurements \cite{stephens1995review}, the use of Eq. (\ref{eq_seq}) is still a common practice\cite{crago2014use,mallick2018bridging}. This gives rise to the question if the short equation (Eq. (\ref{eq_Tseq})) is adequate to estimate LST from ground-based measurements. In the remainder of this paper, we will refer to LST obtained using the long equation (Eq. (\ref{eq_Tleq})) as $T_{leq}$ and to LST obtained using the short equation (Eq. (\ref{eq_Tseq})) as $T_{seq}$.
...
\begin{enumerate}
	\item Can we obtain an adequate estimate of plot-scale LST while neglecting the reflected \DIFdelbegin \DIFdel{down-welling }\DIFdelend \DIFaddbegin \DIFadd{downwelling }\DIFaddend longwave radiation?
...
\begin{figure}[h!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \begin{overpic}[width=0.45\textwidth]{Brookings_se_2005-08} 
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \begin{overpic}[width=0.45\textwidth]{brook_mx_seq} 
		\DIFaddendFL \put (18,57){\textbf{a}}
	\end{overpic}
	\DIFdelbeginFL %DIFDELCMD < \begin{overpic}[width=0.45\textwidth]{Brookings_le_2005-08} %%%
\DIFdelendFL \DIFaddbeginFL \begin{overpic}[width=0.45\textwidth]{book_mx_leq} \DIFaddendFL % ,grid,tics=10
		\put (18,56){\textbf{b}}
	\end{overpic}
...
\begin{figure}[h!]
    ...
    LST was calculated using the short \DIFdelbeginFL \DIFdelFL{or long }\DIFdelendFL equation (Eq. \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL \ref{eq_Tseq}\DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{)) or long equation (}\DIFaddendFL Eq. \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL \ref{eq_Tleq})\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL with landscape-scale emissivity ($\epsilon_{MODIS}$). The median values of $\Delta T$ are shown at the top of the plot and the  \DIFdelbeginFL \DIFdelFL{emissivities }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\epsilon_{MODIS}$ values }\DIFaddendFL used for the $T_{s}$ retrieval are shown at the bottom in orange. See Table \ref{table:studysites} for site abbreviations. \DIFaddbeginFL \DIFaddFL{The shapes of the violin represent the distributions of $\Delta T$ values.}\DIFaddendFL }
...
The uncertainty in plot-scale $\epsilon$ estimated using Eq. \ref{eq_Tleq} ('leq') and Eq. \ref{eq_H} (i.e. without intercept in $H(\Delta  T)$) was mainly in the range of $\pm$0.02 to $\pm$0.05, with a maximum of $\pm$0.2 if outliers are included (blue color in Fig. \ref{fig:eps_unc1}a). The short equation (Eq.\ref{eq_Tseq}, 'seq') resulted in a vary narrow range of $\epsilon$ values between 0.94 and 0.99 throughout the year, with very small uncertainty (around $\pm$0.01, black boxes in Fig. \ref{fig:eps_unc1}a). Interestingly, the differences in $\epsilon$ uncertainty did not propagate into differences in LST uncertainty, which were around $\pm$0.2 K at the hourly scale for each equation if plot-scale emissivity was used (blue boxes in Fig. \ref{fig:eps_unc1}b and black boxes in Fig. \ref{fig:eps_unc1}c). In fact, if landscape-scale values of $\epsilon$ were used, the LST uncertainty was even bigger ($\pm$0.5 K, orange boxes in Fig. \ref{fig:eps_unc1}b and c). \DIFdelbegin \DIFdel{The comparison of uncertainty in $epsilon$ using short equation and the long equation with and without intercept is shown in SI Figure 7. The uncertainty in hourly }\DIFdelend \DIFaddbegin \DIFadd{However, if an intercept in the $H(\Delta  T)$) relationship was allowed during estimation of $\epsilon_{plot}$, the uncertainty in $\epsilon_{plot}$ largely vanished (SI Figure 6a), while the uncertainty in }\DIFaddend $T_{s} -T_{a}$ \DIFdelbegin \DIFdel{more than doubles if an intercept is allowed during the estimation of $\epsilon$ }\DIFdelend \DIFaddbegin \DIFadd{at the diurnal scale doubled }\DIFaddend (SI Figure \DIFdelbegin \DIFdel{7(b)}\DIFdelend \DIFaddbegin \DIFadd{6b}\DIFaddend ).
...
%DIF <  Limitation:surface energy balance closure.
Surface heterogeneity has also been recognized as one of the potential causes for the lack of energy balance closure observed at most ECS at diurnal scales \cite{wilson2002energy, stoy2013data}. However, in our analysis the use of an energy balance closure scheme (based on the Bowen ratio) led to much lower values of \DIFdelbegin \DIFdel{plot-scale $\epsilon$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{plot}$ }\DIFaddend using Holmes approach with the long equation and without intercept. In contrast, if an intercept was allowed, energy balance closure led to an increase in positive intercept (Fig. \ref{fig:mxc_dis}b). Perhaps this is the reason why other studies on plot-scale $\epsilon$ estimation have also used the observed fluxes without correction\cite{holmes2009land,holmes2016cloud,maes2019potential}. Other energy balance closure schemes add the missing energy to H in water limited ecosystems\cite{twine2000correcting}, or to LE in energy limited ecosystems \cite{chakraborty2019biases}. However, our analysis suggests that the footprint mismatch may cause a small bias in the \DIFdelbegin \DIFdel{up-welling }\DIFdelend \DIFaddbegin \DIFadd{upwelling }\DIFaddend longwave radiation measurements that is not accounted in any conventional energy balance closure approaches. When we added 35 $Wm^{-2}$ (instead of 40 $Wm^{-2}$, see Fig. \ref{fig:mxc_dis}a) to the measured \DIFdelbegin \DIFdel{up-welling }\DIFdelend \DIFaddbegin \DIFadd{upwelling }\DIFaddend longwave radiation and subsequently closed the energy balance at the %DIF > SJS: Why did you previously remove the "the"?
HS site (which had the largest $H(\Delta T)$ intercept), we largely removed the intercept and at the same time obtained realistic $\epsilon$ values and an increased $R^2$ (SI Figure 6). In addition, the bias between MODIS LST and plot-scale LST at HS was reduced by 6.4 K (SI Figure \DIFdelbegin \DIFdel{6(b))}\DIFdelend \DIFaddbegin \DIFadd{6b), }\DIFaddend compared to using \DIFdelbegin \DIFdel{up-welling }\DIFdelend \DIFaddbegin \DIFadd{upwelling }\DIFaddend longwave without correction. 

When estimating plot-scale LST using \DIFdelbegin \DIFdel{landscape-scale $\epsilon$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{MODIS}$ }\DIFaddend values, we found at many sites with a sparse canopy strongly negative bias in comparison to MODIS LST, which is in agreement with previous studies where the bias for sparse canopies reached up to 12 $K$ \cite{guillevic2018land}. The MODIS overpass can have a large off-Nadir viewing angle, which would lead to an elongated foot-print\cite{margulis2019joint} and therefore\DIFaddbegin \DIFadd{, }\DIFaddend a different distribution of bare soil and vegetated areas compared to the mostly Nadir viewing angle of the tower-mounted sensor. The difference in footprint and viewing angles between the tower mounted pyrgeometers and MODIS radiometers could also be the reason for bias between the two LST estimates. Plot-scale LST estimates based on plot-scale $\epsilon$ using a linear $H(\Delta T)$ fit without an intercept largely reduced this bias between plot-scale and MODIS LST (Table \ref{table:eps_comp}) and also reduced the uncertainty in diurnal LST (Fig. \ref{fig:eps_unc1}b, c) in comparison to the use of \DIFdelbegin \DIFdel{landscape-scale $\epsilon$}\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{MODIS}$}\DIFaddend . However, the resulting plot-scale $\epsilon$ values were unrealistically low at some sites (Table \ref{table:eps_comp}, center). In contrast, allowing an intercept ($H=m*\Delta T + c$) in \DIFdelbegin \DIFdel{plot-scale $\epsilon$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{plot}$ }\DIFaddend estimation resulted in more realistic $\epsilon$ values at these sites, but very large intercept values (over 200 \DIFdelbegin \DIFdel{W m}\DIFdelend \DIFaddbegin \DIFadd{Wm}\DIFaddend $^{-2}$ at some sites), indicating that the plot-scale LST values cannot be used in combination with the observed aerodynamic fluxes at these sites, as strongly positive $H$ at 0 surface-air temperature difference is physically inconsistent (Fig. \ref{fig:2_mx_c}c). In addition, this approach increased the bias between plot-scale and MODIS LST at most of the study sites (Table \ref{table:eps_comp}). Note that the correspondence between landscape-scale LST and plot-scale LST can vary strongly between sites,  depending on canopy densities and viewing angles (tower vs. satellite)\DIFdelbegin \DIFdel{(Margulis et.al 2019)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{margulis2019joint}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , sensor installation height and position, and sensor types\cite{marcolla2018geometry}. At the sites with the largest intercept values, we found that an assumed bias in \DIFdelbegin \DIFdel{up-welling }\DIFdelend \DIFaddbegin \DIFadd{upwelling }\DIFaddend longwave radiation by only 6-9\% would largely remove the intercept and also reduce the bias between MODIS and plot-scale LST (Fig. \ref{fig:mxc_dis}a, SI Figure \DIFdelbegin \DIFdel{6(b)}\DIFdelend \DIFaddbegin \DIFadd{6b}\DIFaddend ). A detailed analysis of such bias and potential correction approaches is beyond the scope of this study. Given that the fit of a linear model without intercept is statistically questionable in general \cite{eisenhauer2003regression}, and the fact that such a fit resulted in unrealistically low values of $\epsilon$ at some sites, we conclude that fitting a model with intercept is the more robust approach, and that a significant intercept should be used as a red flag for the utility of the data for estimation of plot-scale LST. \DIFaddbegin \DIFadd{Additionally, the uncertainty in $\epsilon_{plot}$ values obtained using a regression model with intercept nearly vanished in comparison to the uncertainty resulting from a regression model without intercept (see SI Figure 6a). 
}\DIFaddend 

Note that the fluxes observed at ECS are representative of the composite signal from both, soil and vegetation, which typically have \DIFdelbegin \DIFdel{a different range }\DIFdelend \DIFaddbegin \DIFadd{different ranges }\DIFaddend of surface temperatures and emissivities\cite{jin2006improved}. The $\epsilon$ of soil strongly depends on soil moisture content \cite{mira2007influence}, whereas the $\epsilon$ of a canopy depends on its structural attributes and leaf area index, the latter of which can vary strongly at the seasonal scale \cite{chen2015determining}. For example, the laboratory-measured directional $\epsilon$ for various canopy elements (bark, leaf and its arrangement, stem wood) ranged between 0.9 to 1 \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{vishnevetsky2019method}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{at the Yatir site \mbox{%DIFAUXCMD
\cite{vishnevetsky2019method}}\hspace{0pt}%DIFAUXCMD
. %DIF > SJS: I added "at the Yatir site" to clarify. Is this correct?
 }\DIFaddend Laboratory measurements of thermal infrared reflectance spectra suggest that the $\epsilon$ uncertainty due to structural unknowns, such as leaf orientation, is more significant than the differences in leaf component emissivity among plant species \cite{snyder1998classification}. Consequently, it is clear that the $\epsilon$ of a surface is a function of many factors and \DIFdelbegin \DIFdel{detailed study }\DIFdelend \DIFaddbegin \DIFadd{a detailed analysis }\DIFaddend of all these factors is out of scope of the present study. Derivation of landscape-scale broadband \DIFdelbegin \DIFdel{$\epsilon_{land}$ }\DIFdelend \DIFaddbegin \DIFadd{emissivity ($\epsilon_{MODIS}$) }\DIFaddend from narrowband spectral emissivity is a first-order approximation for capturing the integrated effects of land cover from MODIS spectral bands\cite{jin2006improved}, whereas the derivation of \DIFdelbegin \DIFdel{plot-scale $\epsilon$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{plot}$ }\DIFaddend from EC flux data \DIFdelbegin \DIFdel{, as analyzed in this study, }\DIFdelend provides an independent \DIFdelbegin \DIFdel{, alternative estimate of }\DIFdelend \DIFaddbegin \DIFadd{alternative for the estimation of effective plot-scale }\DIFaddend $\epsilon$\DIFdelbegin \DIFdel{and at the same time provides a quality check on the correspondence }\DIFdelend \DIFaddbegin \DIFadd{. Our finding that inclusion of an intercept in the $H(\Delta T)$ relationship when estimating $\epsilon_{plot}$ significantly reduces uncertainty in $\epsilon_{plot}$ while increasing uncertainty in $\Delta T$ suggests that this method could be used for reliable estimates of effective $\epsilon_{plot}$ within the radiometer footprint even in the presence of a footprint mismatch between the radiometric and $H$ measurements. The approach could also be extended to urban settings if reliable eddy covariance measurements are available and anthropogenic heat components are known. Although the effects of footprint mismatch }\DIFaddend between radiometric and \DIFdelbegin \DIFdel{aerodynamic measurements at a given ECS}\DIFdelend \DIFaddbegin \DIFadd{eddy covariance measurements could be large in such a heterogeneous setting, $\epsilon_{plot}$ estimation based on $H(\Delta T)$ with intercept could provide a robust estimate of effective $\epsilon_{plot}$, which is important for climate models simulating urban heat island effects \mbox{%DIFAUXCMD
\cite{chakraborty2021land}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 


In summary, our results reveal that the short equation (Eq. (\ref{eq_Tseq}), neglecting \DIFdelbegin \DIFdel{down-welling }\DIFdelend \DIFaddbegin \DIFadd{downwelling }\DIFaddend longwave radiation) leads to biased estimates of LST and  \DIFdelbegin \DIFdel{hugely }\DIFdelend \DIFaddbegin \DIFadd{substantially }\DIFaddend over-estimated sensitivity of LST to surface emissivity. Therefore, the use of Eq. (\ref{eq_Tseq}) is not recommended and should be replaced by Eq. (\ref{eq_Tleq}) if \DIFdelbegin \DIFdel{down-welling }\DIFdelend \DIFaddbegin \DIFadd{downwelling }\DIFaddend longwave radiation measurements are available. At some sites, the use of Eq. (\ref{eq_Tleq}) resulted in plot-scale LST estimates that were far below satellite-derived landscape-scale LST values, and also inconsistent with plot-scale flux data (negative surface-air temperature difference when sensible heat flux is strongly positive). In many previous studies, such bias would have been removed by slightly lowering surface emissivity ($\epsilon$), but the reduced sensitivity of Eq. (\ref{eq_Tleq}) to $\epsilon$ would require unrealistically low values of $\epsilon$ to remove the low-bias in LST. When estimating plot-scale $\epsilon$ values, realistic estimates based on Eq. (\ref{eq_Tleq}) are only possible at these sites if we include an intercept in the $H (\Delta T)$ relationship, but this again results in very high intercept values (over 200 W m$^{-2}$). \DIFdelbegin \DIFdel{However, assuming }\DIFdelend \DIFaddbegin \DIFadd{Note that high values of intercept do not necessarily make $\epsilon_{plot}$ unreliable, they rather suggest poor correspondence between $H$ and $T_{s}$ due to footprint mismatch. Hypothesizing }\DIFaddend that the intercept is a consequence of a foot print mismatch between the aerodynamic and radiometric measurements, a small correction in \DIFdelbegin \DIFdel{up-welling }\DIFdelend \DIFaddbegin \DIFadd{upwelling }\DIFaddend longwave (6-9\%) and subsequent energy balance closure (based on \DIFaddbegin \DIFadd{the }\DIFaddend Bowen ratio) largely removed the intercept and produced realistic \DIFdelbegin \DIFdel{$\epsilon$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{plot}$ }\DIFaddend values and self-consistent $H(\Delta T)$ plots. This approach also reduced the bias between plot-scale LST and MODIS LST, although it did not improve the weak correlation between \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{these LST estimates }\DIFaddend (SI Figure \DIFdelbegin \DIFdel{6}\DIFdelend \DIFaddbegin \DIFadd{5}\DIFaddend (b)). \DIFaddbegin \DIFadd{In the past, ground-based radiometric measurements have been used for the validation of the MODIS LST product \mbox{%DIFAUXCMD
\cite{wang2008validating}}\hspace{0pt}%DIFAUXCMD
, whereas here we compared the plot-scale LST with MODIS LST to check its dependency.
%DIF > Unfortunately, we are currently not in the position to predict a priori at which sites (ecosystems) a low correlation between plot-scale and landscape-scale LST should be expected. %SJS: I would remove this sentence, as we did not even attempt to explain why such a low correlation occurs at some sites.
}

%DIF > This method also reduces the uncertainty in $T_{s}$ due of uncertainty in $\epsilon$ only rather it

  \DIFadd{The combination of radiometric and aerodynamic measurements for the estimation of $\epsilon_{plot}$ and LST provides a quality check on the correspondence between observed fluxes and temperatures at ECS. }\DIFaddend The intercept value can be used as a \DIFdelbegin \DIFdel{criterion to check the consistency of }\DIFdelend \DIFaddbegin \DIFadd{consistency criterion for }\DIFaddend observed data (radiometric and aerodynamic measurements) before using them in combination, as a strong intercept indicates inconsistency between observed sensible heat flux and surface-to-air temperature difference. Therefore, the proposed method of fitting a linear relation with intercept to $H$ and $\Delta T$ \DIFdelbegin \DIFdel{data }\DIFdelend has the potential to provide more \DIFdelbegin \DIFdel{robust }\DIFdelend \DIFaddbegin \DIFadd{reliable }\DIFaddend benchmark data sets for model evaluation and validation at the \DIFaddbegin \DIFadd{ecosystem scale (}\DIFaddend plot-scale\DIFdelbegin \DIFdel{. Furthermore, the study }\DIFdelend \DIFaddbegin \DIFadd{). The $\epsilon_{plot}$ estimates }\DIFaddend could also be \DIFdelbegin \DIFdel{extended for urban settings with eddy covariance measurements and known anthropogenic heat components. Unfortunately, we are currently not in the position to predict a priori at which sites a low correlation between plot-scale and landscape-scale LST should be expected}\DIFdelend \DIFaddbegin \DIFadd{used to parameterize climate and weather prediction models at ecosystem scale, but this was not tested in the present study}\DIFaddend . Overall, the implications of our study are of particular relevance for the research community interested in \DIFdelbegin \DIFdel{understanding }\DIFdelend \DIFaddbegin \DIFadd{process-based understanding of }\DIFaddend the diurnal and seasonal \DIFdelbegin \DIFdel{feedback }\DIFdelend \DIFaddbegin \DIFadd{feedbacks }\DIFaddend in soil-vegetation systems based on \DIFdelbegin \DIFdel{fluxes, emissivity and LST estimated at the plot-scale}\DIFdelend \DIFaddbegin \DIFadd{observed fluxes}\DIFaddend .
 ...
 \textbf{Tower data}:
ECS collect micro-meteorological measurements above the surface (vegetation canopy) using towers (flux tower) following common measurement protocols \cite{baldocchi2001fluxnet}. The towers are generally equipped with an instrument made up of pyrgeometers or radiometers to measure \DIFdelbegin \DIFdel{up-welling and down-welling }\DIFdelend \DIFaddbegin \DIFadd{upwelling and downwelling }\DIFaddend shortwave and longwave radiation, which is further used to calculate net radiation (Eq. \ref{eq_Rn1}). Besides radiative fluxes, measurement at ECS also include sensible and latent heat fluxes, net carbon-dioxide exchange and a range of meteorological variables, such as air temperature \DIFaddbegin \DIFadd{($T_{a}$)}\DIFaddend , humidity and wind speed. $T_{a}$ is the air temperature measured at a reference height above the canopy. Each flux measurement is accompanied by a flagging system based on the second CarboEurope-IP QA/QC workshop \cite{gilberto2020fluxnet2015}. In our current work\DIFdelbegin \DIFdel{we use available high quality }\DIFdelend \DIFaddbegin \DIFadd{, we use high quality available data }\DIFaddend (flag 0) \DIFdelbegin \DIFdel{data }\DIFdelend as it is without atmospheric corrections. %DIF > SJS: This sounds like a bad thing, as in a missing correction. Perhaps better: "as it does not require atmospheric correction"?
...
\textbf{MODIS data}:
Landscape-scale emissivity and LST data (MODIS product MOD11A1) was downloaded from NASA earth data \href{https://lpdaac.usgs.gov/}. It is a level 3 daily LST product gridded in the sinusoidal projection at a spatial resolution of 0.928 \DIFdelbegin \DIFdel{$km$ }\DIFdelend \DIFaddbegin \DIFadd{km }\DIFaddend by 0.928 \DIFdelbegin \DIFdel{$km$}\DIFdelend \DIFaddbegin \DIFadd{km}\DIFaddend . The daily LST pixel values in each \DIFdelbegin \DIFdel{granules }\DIFdelend \DIFaddbegin \DIFadd{granule }\DIFaddend (tile contains 1200 x 1200 grids in 1200 rows and 1200 columns) is retrieved by the generalized split-window algorithm under clear-sky conditions and MODIS LST values are averaged by overlapping pixels in each grid with overlapping areas as weight\cite{wan2007collection}. The downloaded data in hierarchical data format (hdf), were converted into tagged image file format (tiff) using \DIFdelbegin \DIFdel{a python package called }\DIFdelend \DIFaddbegin \DIFadd{the python package }\DIFaddend PyModis \cite{delucchi2014pymodis}. \DIFdelbegin \DIFdel{From tiff the files are converted into csv format. The details of data extraction and conversion can be found at }%DIFDELCMD < \url{https://renkulab.io/projects/gitanjali.thakur/modis_lst_fpar/}%%%
\DIFdel{. Thermal remote sensing (MODIS ) is used to measure }\DIFdelend \DIFaddbegin \DIFadd{Alternatively MODIS data can also be obtained from }\url{ https://appeears.earthdatacloud.nasa.gov/}\DIFadd{. MODIS measures }\DIFaddend spectral emissivity through four channels (28, 29, 30, 31) at wavelengths ranging between 8-12 $\mu$m \cite{jin2006improved} and the system of equations is iteratively solved for a given range of wavelengths (\DIFdelbegin \DIFdel{9 }\DIFdelend \DIFaddbegin \DIFadd{8 }\DIFaddend - 12 $\mu$m) to obtain $\epsilon$ and LST using radiative transfer models\cite{hulley2012quantifying,jin2006improved,wang2009evaluation}\DIFaddbegin \DIFadd{. }\DIFaddend In the current study,  dataset columns used to compare plot-scale LST are \DIFdelbegin \DIFdel{: }\DIFdelend day time daily LST and local view time. In order to obtain landscape-scale $\epsilon$\DIFaddbegin \DIFadd{, }\DIFaddend the emissivity from bands 31 and 32 are used. These bands have \DIFdelbegin \DIFdel{more stable emissivities }\DIFdelend \DIFaddbegin \DIFadd{stable emissivities than other channels }\DIFaddend ranging from ~0.92-1, \DIFdelbegin \DIFdel{thus }\DIFdelend \DIFaddbegin \DIFadd{and can be }\DIFaddend used to derive broadband emissivity \cite{wan2007collection}. 

\begin{table}[h!]
\centering
\caption{Description of study sites}
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{|p{2.5cm}|p{1.5cm}|p{2.2cm}|p{1.5cm}|p{2cm}|p{1cm}|p{1cm}|}
%DIFDELCMD <  %%%
...
%DIF >  with $H$, $R_{lup}$, $T_{a}$
%DIF > Monthly observation of $H$, $R_{lup}$, $R_{ldwn}$, and $T_{a}$ satisfying the filtering criteria are segregated.
\textbf{Plot-scale $\epsilon$ and LST \DIFdelbegin \DIFdel{etimation}\DIFdelend \DIFaddbegin \DIFadd{estimation}\DIFaddend }:
LST is \DIFaddbegin \DIFadd{defined as }\DIFaddend the “ensemble directional radiometric surface temperature” \cite{norman1995terminology}, and can be estimated from the infrared radiance emanating from a given surface with known emissivity\cite{kustas2007utility}. \DIFdelbegin \DIFdel{We solve Eq. (\ref{eq_Tleq}) and Eq. (\ref{eq_Tseq}) measured longwave and known surface emissivity (we start with maximum possible value for grey body, 0.99) . This }\DIFdelend \DIFaddbegin \DIFadd{The  emissivity at ecosystem-scale can also be estimated using observed $H$, $R_{lup}$, $R_{ldwn}$, and $T_{a}$. A plot-scale $\epsilon$ ($\epsilon_{plot}$) estimation }\DIFaddend approach was initially proposed by Holmes\cite{holmes2009land} \DIFdelbegin \DIFdel{to estimate plot-scale $\epsilon$ }\DIFdelend using the short equation (Eq. \ref{eq_Tseq})\DIFdelbegin \DIFdel{with $H$, $R_{lup}$, $T_{a}$}\DIFdelend . In the present work, we have used both the long equation (Eq. \ref{eq_Tleq}) and \DIFaddbegin \DIFadd{the }\DIFaddend short equation (Eq. \ref{eq_Tseq}) to estimate \DIFdelbegin \DIFdel{$\epsilon$}\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{plot}$}\DIFaddend . The prime variables used in the study are $H$, $R_{lup}$, $R_{ldwn}$, and \DIFdelbegin \DIFdel{, }\DIFdelend $T_{a}$, whereas the ancillary variables $R_{n}$ and wind speed (\DIFdelbegin \DIFdel{$Ws$}\DIFdelend \DIFaddbegin \DIFadd{$W_{s}$}\DIFaddend ) are used to filter the data for analysis. The data filtering criteria are \DIFaddbegin \DIFadd{sufficient net radiation }\DIFaddend ($R_{n} > 25$ $Wm^{-2}$) and wind speed (\DIFdelbegin \DIFdel{$Ws > 2ms^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$W_{s} > 2ms^{-1}$}\DIFaddend )\cite{holmes2009land}. \DIFdelbegin \DIFdel{Data ($R_{lup}$, $R_{ldwn}$, $H$, $T_{a}$) satisfying the filtering criteria is segregated monthly. 
Assuming an initial $\epsilon$ value, observed longwave radiation ($R_{lup}$, $R_{ldwn}$) is used to estimate $T_{s}$ for each valid data point within a month. }\DIFdelend For each month, a linear regression (with \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend without intercept, see main text) \DIFdelbegin \DIFdel{is performed }\DIFdelend between sensible heat ($H$) and \DIFdelbegin \DIFdel{surface-air temperature difference (}\DIFdelend $T_{s}-T_{a}$) \DIFaddbegin \DIFadd{is performed }\DIFaddend (Fig. \ref{fig:flow_chart}b) using Scipy \DIFdelbegin %DIFDELCMD < \url{https://docs.scipy.org/doc/scipy-0.16.0/reference/generated/scipy.stats.linregress.html}%%%
\DIFdel{. This procedure is repeated using a predefined range of }\DIFdelend \DIFaddbegin \DIFadd{(}\url{https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.linregress.html}\DIFadd{). $T_{s}$ is estimated by solving Eq. (\ref{eq_Tleq}) and Eq. (\ref{eq_Tseq}) using measured longwave radiation and prescribed }\DIFaddend $\epsilon$\DIFdelbegin \DIFdel{from 0.4 until 0.998 with a }\DIFdelend \DIFaddbegin \DIFadd{, starting with the maximum possible value for a grey body, 0.99, and then progressively reducing $\epsilon$ with }\DIFaddend step size of 0.002 \DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{From all the values of $\epsilon$ that resulted in strong correlation }\DIFdelend \DIFaddbegin \DIFadd{until we reach a minimum RMSE for a linear relationship }\DIFaddend between $H$ and \DIFdelbegin \DIFdel{$T_{s}-T_{a}$ (}\DIFdelend \DIFaddbegin \DIFadd{$\Delta T$. Only months }\DIFaddend with $R^{2}>0.5$ \DIFdelbegin \DIFdel{), the value that achieved the smallest RMSE was chosen as the suitable $\epsilon$ for the given month}\DIFdelend \DIFaddbegin \DIFadd{between $H$ and $\Delta T$ are considered for $\epsilon_{plot}$ estimation}\DIFaddend . An illustration plot for RMSE \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{as a function of }\DIFaddend $\epsilon$ is shown in SI \DIFdelbegin \DIFdel{Figure5. The $\epsilon$ is obtained for each month using both, }\DIFdelend \DIFaddbegin \DIFadd{Figure 5. The monthly $\epsilon_{plot}$ is obtained using the }\DIFaddend long (Eq. (\ref{eq_Tleq})) and short equation (Eq.(\ref{eq_Tseq})) and termed as $\epsilon_{leq}$ and $\epsilon_{seq}$ respectively, as shown in Fig. \ref{fig:flow_chart}b. For \DIFdelbegin \DIFdel{sites like HS and LF resulting in }\DIFdelend \DIFaddbegin \DIFadd{two sites with }\DIFaddend a high value of intercept \DIFdelbegin \DIFdel{, we hypothesized an underestimation of $R_{lup}$ and corrected it by adding $6-8\%$ of the observed day time observed }\DIFdelend \DIFaddbegin \DIFadd{(HS and LF in Table \ref{table:eps_comp}) we tested if adding 6-8\% to the observed }\DIFaddend $R_{lup}$ \DIFdelbegin \DIFdel{to the measured data before }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend closing the energy balance using Bowen ratio closure \DIFdelbegin \DIFdel{, and estimating monthly plot-scale $\epsilon$. }\DIFdelend \DIFaddbegin \DIFadd{before $\epsilon_{plot}$ estimation would remove the intercept (Fig. \ref{fig:mxc_dis}). %DIF > SJS: I re-arranged and re-wrote the above a bit. It sounded as if you proposed to add 6-8% to R_lup for all sites with a large intercept as part of the routine, which I don't think is what you wanted to convey.
}

\DIFaddend Recently, another approach for plot-scale $\epsilon$ estimation using Eq. (\ref{eq_Rlup}) was \DIFdelbegin \DIFdel{discussed }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend by Maes et.al (2019)\cite{maes2019potential}. 
\DIFdelbegin \DIFdel{For each month, the observed $R_{lup}$,  $R_{ldwn}$, $T_{a}$  }\DIFdelend \DIFaddbegin \DIFadd{In this approach, data sets  }\DIFaddend are filtered for non rainy days \DIFdelbegin \DIFdel{using the filtering criteria ($-2 < H <2$ , and }\DIFdelend \DIFaddbegin \DIFadd{without snow cover (}\DIFaddend $\alpha < 0.4$) \DIFaddbegin \DIFadd{and near-zero $H$ ($-2 < H <2$)}\DIFaddend . The $\epsilon$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{values are then }\DIFaddend estimated by substituting $T_{s} = T_{a}$ in Eq. (\ref{eq_Rlup}) as shown in Eq. (\ref{eq_e_Maes}). The monthly $\epsilon$  was \DIFdelbegin \DIFdel{represented }\DIFdelend \DIFaddbegin \DIFadd{obtained }\DIFaddend as the median of $\epsilon$ obtained by substituting filtered data in Eq. (\ref{eq_e_Maes}) \DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{(red stars in Fig. \ref{fig:HDT}). %DIF > SJS: I re-formulated the above, please check.
}\DIFaddend \begin{equation}\label{eq_e_Maes}
\epsilon = \frac{R_{ldwn} - R_{lup}}{R_{ldwn} - T_{a}^{4} \sigma}
\end{equation}

\textbf{LST comparison}: MODIS LSTs are a global reference for LST and used world-wide, also in conjuction with plot-scale flux measurements. \DIFdelbegin \DIFdel{Plot-scale LST based on optimum $\epsilon$ for each month using a linear regression with and without intercept $H(\Delta T)$ is compared to the MODIS daily LST (without correcting for the viewing angle). For each month the $T_{s}$ estimates using plot-scale $\epsilon$ and MODIS $\epsilon$ corresponding to the exact }\DIFdelend \DIFaddbegin \DIFadd{To calculate plot-scale LST for the exact time of }\DIFaddend TERRA day-time \DIFdelbegin \DIFdel{of pass was obtained (}\DIFdelend \DIFaddbegin \DIFadd{overpass for each site, the }\DIFaddend 30 minute tower data was \DIFdelbegin \DIFdel{upsampled to each minute using linear interpolation). TERRA (satellite) overpasses at local solar time between 10:30 am to 12 pm in ascending mode \mbox{%DIFAUXCMD
\cite{guillevic2017land}}\hspace{0pt}%DIFAUXCMD
. Plot-scale LST at local satellite overpass ( $T_{leq}$ and $T_{seq}$) was obtained using measured longwave radiation that was interpolated to match the exact }\DIFdelend \DIFaddbegin \DIFadd{interpolated linearly, and the interpolated $R_{ldwn}$ and $R_{lup}$ observations corresponding to the }\DIFaddend time of overpass \DIFdelbegin \DIFdel{and corresponding monthly plot-scale $\epsilon$.
}\DIFdelend \DIFaddbegin \DIFadd{were used in conjunction with the monthly $\epsilon_{plot}$ or $\epsilon_{MODIS}$ for the calculation.
%DIF > SJS: Above, I tried to re-organise and clarify the following text, which confused the hell out of me: 
}\DIFaddend Plot-scale daily LST is compared to MODIS LST in terms of \DIFaddbegin \DIFadd{the }\DIFaddend mean, bias, RMSE and $R^2$ using a robust linear regression model (scipy stat model\DIFdelbegin \DIFdel{,}%DIFDELCMD < \url{https://github.com/scipy/scipy/blob/v1.7.1/scipy/stats/_stats_mstats_common.py} 
%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \DIFadd{) %DIF > SJS: Don't you need to provide a reference fo the Scipy stats model?
}\DIFaddend as shown in Fig. \ref{fig:flow_chart}a. The goodness of fit between plot-scale and landscape-scale LST was determined by looking at $R^2$ (Fig. \ref{fig:flow_chart}b). The bias is estimated as the mean of \DIFaddbegin \DIFadd{the }\DIFaddend deviation between daily MODIS LST and \DIFdelbegin \DIFdel{ground-based $T_{s}$ ($T_{leq,seq} - T_{MODIS}$). In past ground based radiometric measurement have been used for validation of MODIS LST product \mbox{%DIFAUXCMD
\cite{wang2008validating} }\hspace{0pt}%DIFAUXCMD
whereas the we comapred the }\DIFdelend plot-scale \DIFdelbegin \DIFdel{LST with MODIS LST as sanity check. }\DIFdelend \DIFaddbegin \DIFadd{$T_{s}$. %DIF > SJS: I removed seq,leq, just confusing here.
%DIF > SJS: The following does not belong in the methods, but in the disucssion, so I commented it out: In the past, ground-based radiometric measurements have been used for validation of the MODIS LST product \cite{wang2008validating}, whereas here we compared the plot-scale LST with MODIS LST as a sanity check. 
}\DIFaddend See SI Table 1 for data sources and acronyms.
...
Uncertainty in plot-scale $\epsilon$ and LST was quantified based on an assumed systematic error (caused by a potential bias in measurement devices) at the study sites. In a first step, based on the literature \cite{trenberth2012tracking,foken2008energy}\DIFaddbegin \DIFadd{, }\DIFaddend the error bounds of each input variable ($ H$, $R_{lup}$, $R_{ldw}$, $T_{a}$) used for plot-scale $\epsilon$ estimation were defined. The error bounds for $R_{lup}$ and $R_{ldwn}$ are -5 to 5 $Wm^{-2}$ \cite{trenberth2012tracking}, for $H$, we used -20 to 20 $Wm^{-2}$ and for $T_{a}$ we used -1 to 1 $K$ \cite{foken2008energy}. The error samples (perturbation) within these bounds were generated using the Saltelli sampling scheme (using \DIFdelbegin \DIFdel{python based }\DIFdelend \DIFaddbegin \DIFadd{the python }\DIFaddend package SALIB\cite{saltelli2017new}). Each error sample is added to the monthly segregated measured fluxes as explained above. Observed fluxes combined with perturbed fluxes are used to estimate $T_{s}$ using Eq. (\ref{eq_Tseq}) and  Eq. (\ref{eq_Tleq}). The obtained range of diurnal $T_{s}$ and observed $T_{a}$ \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{based on the }\DIFaddend perturbation is used to calculate the \DIFdelbegin \DIFdel{uncertainity }\DIFdelend \DIFaddbegin \DIFadd{uncertainty }\DIFaddend in $\Delta T$; an example for July 15 is shown in Fig. \ref{fig:eps_unc1}c. Perturbed sensible heat flux ($H + $sample error) and perturbed $\Delta T$ is used to obtain \DIFdelbegin \DIFdel{$\epsilon$ as described in plot-scale $\epsilon$ estimation. The range of monthly plot-scale $\epsilon$ obtained }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{plot}$ as described above. The distribution %DIF > SJS: Not just the range, but the distribution, right?
 of monthly $\epsilon_{plot}$ values }\DIFaddend is reported as uncertainty in monthly $\epsilon$. 
\section{Author contributions statement}
GT, SJS, KM, and IT designed the study. GT conducted the analysis and prepared the manuscript together with SJS. KM, IT and MS provided textual input, corrections, suggestions and critical discussion of the manuscript. \DIFaddbegin \DIFadd{The author order reflects roughly the level of contribution to this paper.
}\DIFaddend \section{Code and data availability}
The data and code used for this study is \DIFdelbegin \DIFdel{uploaded to }\DIFdelend \DIFaddbegin \DIFadd{freely available from }\DIFaddend zenodo.org (\url{https://doi.org/10.5281/zenodo.6385016})\DIFaddbegin \DIFadd{.
}\DIFaddend 
...
\centering
\DIFdelbeginFL %DIFDELCMD < \scalebox{1}{
%DIFDELCMD < \begin{tikzpicture}[node distance=2cm]
...
%DIFDELCMD < \draw [arrow] (start) -- (in1);
%DIFDELCMD < \draw [arrow] (in1) -- (pro1);
%DIFDELCMD < \draw [arrow] (pro1) -- (dec1);
%DIFDELCMD < \draw [arrow] (start1) -- node[anchor=east] {} (dec1);
%DIFDELCMD < \draw [arrow] (start1) |- (in1);
%DIFDELCMD < \draw [arrow] (dec1) -- (out1);
%DIFDELCMD < \node (txt1) [textbox, below of=out1,yshift= 0.5cm] {\textbf{a}};
%DIFDELCMD < \end{tikzpicture}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \scalebox{0.75}{
...
\begin{subfigure}{.5\textwidth}
\centering
\DIFdelbeginFL %DIFDELCMD < \scalebox{0.75}{
%DIFDELCMD < \begin{tikzpicture}[node distance=2cm]
...
%DIFDELCMD < node[anchor=north] {yes} (start);
...
%DIFDELCMD < %\node (stop) [startstop, below of=out1] {Stop};
%DIFDELCMD < \draw [arrow] (start) -- (in1);
%DIFDELCMD < \draw [arrow] (in1) --  node[anchor=south,xshift=1cm, yshift=-0.5cm] {Eq. \ref{eq_Tleq},Eq. \ref{eq_Tseq}} (pro1);
%DIFDELCMD < \draw [arrow] (pro1) -- node[anchor=south,xshift=1.9cm, yshift=-0.3cm] {$H$ and $\Delta T$ linearfit} (dec1);
%DIFDELCMD < \draw [arrow] (pro1) -- node[anchor=south,xshift=2.1cm, yshift=-0.9cm] { with and without intercept} (dec1);
%DIFDELCMD < \draw [arrow] (dec1) -- node[anchor=east] {yes} (pro2a);
%DIFDELCMD < \draw [arrow] (dec1) -- node[anchor=south] {no} (pro2b);
%DIFDELCMD < \draw [arrow] (pro2b) |- (pro1);
%DIFDELCMD < \draw [arrow] (pro2a) -- (out1);
%DIFDELCMD < \draw [arrow] (out1) -- (dec2);
%DIFDELCMD < \draw [arrow] (dec2) -- (out2);
%DIFDELCMD < \draw [arrow] (start1) -- node[anchor=east]{} (dec2);
...
\end{tikzpicture}}
\DIFaddendFL \end{subfigure}
\caption{Schematic representation of steps followed for plot-scale LST retrieval using \DIFdelbeginFL \DIFdelFL{eddy covariance measurement }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{landscape-scale emissivity }\DIFaddendFL (a) \DIFdelbeginFL \DIFdelFL{Landscape }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{or plot-scale }\DIFaddendFL emissivity \DIFdelbeginFL \DIFdelFL{and longwave measurement and compared to Landscape-scale LST }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{$T_{MODIS}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{b}\DIFaddendFL )\DIFaddbeginFL \DIFaddFL{. To estimate plot-scale $\epsilon_{plot}$, surface-air temperature difference }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{b}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Delta T$}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{Plot-scale emissivity estimation using }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is computed from }\DIFaddendFL observed \DIFdelbeginFL \DIFdelFL{$H$, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{longwave radiation ($R_{lup}$ and }\DIFaddendFL $R_{ldwn}$\DIFaddbeginFL \DIFaddFL{) and $T_{a}$ for given $\epsilon_{plot}$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$R_{lup}$ }\DIFdelendFL and \DIFdelbeginFL \DIFdelFL{plot-scale LST is estimated using plot-scale $\epsilon$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{then $\epsilon_{plot}$ }\DIFaddendFL is \DIFdelbeginFL \DIFdelFL{compared }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{varied in a way }\DIFaddendFL to \DIFdelbeginFL \DIFdelFL{landscape-scale emissivity}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{minimise RMSE of a linear relationship between observed sensible heat flux (H) and $\Delta T$}\DIFaddendFL . The \DIFdelbeginFL \DIFdelFL{$R^{2}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{resulting surface temperatures ($T_{leq}, T_{seq}$) are then compared to $T_{MODIS}$, with the $R^2$}\DIFaddendFL , RMSE, \DIFdelbeginFL \DIFdelFL{Bias are mentioned }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and bias reported }\DIFaddendFL in Fig. (\ref{fig:LST local and MODIS}). 
}
\DIFaddbeginFL 

\DIFaddendFL \label{fig:flow_chart}
\end{figure}
