...
%DIF ADD LST_revis_round2.tex   Tue Apr 26 10:32:06 2022
...
LST is the “ensemble directional radiometric surface temperature” \cite{norman1995terminology}, and can be estimated from the infrared radiance emanating from a given surface with known emissivity\cite{kustas2007utility}. We solve Eq. (\ref{eq_Tleq}) and Eq. (\ref{eq_Tseq}) measured longwave and known surface emissivity (we start with maximum possible value for grey body, 0.99). This approach was initially proposed by Holmes\cite{holmes2009land} to estimate plot-scale $\epsilon$ using the short equation (Eq. \ref{eq_Tseq}) with $H$, $R_{lup}$, $T_{a}$. In the present work, we have used both the long equation (Eq. \ref{eq_Tleq}) and short equation (Eq. \ref{eq_Tseq}) to estimate $\epsilon$. The prime variables used in the study are $H$, $R_{lup}$, $R_{ldwn}$, and, $T_{a}$, whereas the ancillary variables $R_{n}$ and wind speed ($Ws$) are used to filter the data for analysis. The data filtering criteria are ($R_{n} > 25$ $Wm^{-2}$) and wind speed ($Ws > 2ms^{-1}$)\cite{holmes2009land}. Data ($R_{lup}$, $R_{ldwn}$, $H$, $T_{a}$) satisfying the filtering criteria \DIFdelbegin \DIFdel{is segregated monthly. Assuming an initial }\DIFdelend \DIFaddbegin \DIFadd{are segregated for each  month. Starting }\DIFaddend $\epsilon$ value \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{of 0.4 is used with }\DIFaddend observed longwave radiation ($R_{lup}$, $R_{ldwn}$) is used to estimate $T_{s}$\DIFdelbegin \DIFdel{for each valid data point within a month}\DIFdelend \DIFaddbegin \DIFadd{, $T_{s}-T_{a}$ is estimated ($T_{a}$ is measured)}\DIFaddend . For each month, a linear regression (with or without intercept, see main text) is performed between sensible heat ($H$) and surface-air temperature difference ($T_{s}-T_{a}$) (Fig. \ref{fig:flow_chart}b) using Scipy \url{https://docs.scipy.org/doc/scipy-0.16.0/reference/generated/scipy.stats.linregress.html}. 
This procedure is repeated using a predefined range of $\epsilon$ from 0.4 \DIFdelbegin \DIFdel{until }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend 0.998\DIFdelbegin \DIFdel{with a }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend step size of 0.002.

From all the values of $\epsilon$ that resulted in strong correlation between $H$ and $T_{s}-T_{a}$ (with $R^{2}>0.5$), the value that achieved the smallest RMSE was chosen as the \DIFdelbegin \DIFdel{suitable $\epsilon$ for the }\DIFdelend \DIFaddbegin \DIFadd{reliable emissivity value ($\epsilon_{plot}$) for a }\DIFaddend given month. An illustration plot for RMSE and $\epsilon$ is shown in SI \DIFdelbegin \DIFdel{Figure5. The $\epsilon$ }\DIFdelend \DIFaddbegin \DIFadd{Fig. 5. The $\epsilon_{plot}$ }\DIFaddend is obtained for each month using both, long (Eq. (\ref{eq_Tleq})) and short equation (Eq.(\ref{eq_Tseq})) and termed as $\epsilon_{leq}$ and $\epsilon_{seq}$ respectively, as shown in Fig. \ref{fig:flow_chart}b. For sites like HS and LF \DIFdelbegin \DIFdel{resulting in }\DIFdelend \DIFaddbegin \DIFadd{having }\DIFaddend a high value of intercept, we hypothesized an underestimation of $R_{lup}$ and corrected it by adding $6-8\%$ of the observed \DIFdelbegin \DIFdel{day time observed }\DIFdelend $R_{lup}$ to the measured \DIFdelbegin \DIFdel{data before closing the energy balance using Bowen ratio closure, and estimating monthly plot-scale $\epsilon$}\DIFdelend \DIFaddbegin \DIFadd{$R_{lup}$. After correcting $R_{lup}$, energy imbalance is closed for each month and monthly $\epsilon_{plot}$ is obtained}\DIFaddend . 
...
\end{tikzpicture}}
\DIFaddendFL \end{subfigure}
\caption{Schematic representation of steps followed for plot-scale LST retrieval using eddy covariance measurement (a) Landscape emissivity and longwave measurement and compared to Landscape-scale LST ($T_{MODIS}$) (b) Plot-scale emissivity estimation \DIFaddbeginFL \DIFaddFL{$\epsilon_{plot}$ }\DIFaddendFL using observed $H$, $R_{ldwn}$, $R_{lup}$ and plot-scale LST is estimated using \DIFdelbeginFL \DIFdelFL{plot-scale $\epsilon$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\epsilon_{plot}$ and $\epsilon_{MODIS}$ }\DIFaddendFL is compared \DIFdelbeginFL \DIFdelFL{to landscape-scale emissivity. The }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{using }\DIFaddendFL $R^{2}$, RMSE, Bias are mentioned in Fig. (\ref{fig:LST local and MODIS}).}
