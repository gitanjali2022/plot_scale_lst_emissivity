...
%SJS: I am not sure what you want to show with the following statement. Perhaps remove it, and also remove (Fig. \ref{fig:yr_intrc})?
% The variation of maximum recorded $H$ with the estimated intercept for Tumbarumba is also shown in \textbf(SI (Fig. \ref{fig:yr_intrc}). 
...
Interestingly, adding 40 W m$^-{2}$ to the  measured upwelling longwave radiation and subsequent energy balance closure largely removes the intercept and at the same time increases $R^2$, as shown in \textbf(SI Fig. \ref{fig:Rlup_ebc}). %SJS: I moved the last sentence up and reformuated a bit. Please check. What happened to the bias between MODIS and plot-scale LST if you added 40 W and closed the energy balance? Might be worth mentioning, whether this also reduced the bias at HS. 
...
The intercept (i.e. $\Delta T \ne 0$ at $H=0$) could be caused by combining measurements coming from instruments (radiometer, eddy covariance system) with different footprints\cite{marcolla2018geometry}. The mismatch of source areas becomes important if the surface underlying the instruments has heterogeneous land cover. 
 ...
 %SJS: The below does not make sense to me, as I don't see a clear correlation between intercept and Hmax, and I also don't understand the relevance. Could you check?
 %Also, the intercept in $H(\Delta T)$ was proportional to the maximum observed $H$ for each month as shown in Fig. \ref{fig:2_mx_c}. The ratio between $H_{max}$ and intercept varies between $0\%$ to $30 \%$ for TUM (\textbf{SI 7} Fig. \ref{fig:yr_intrc}). 
...
% However, due to general lack of emissivity information for natural ecosystem at ECS, satellite derived emissivity serves as plausible estimates and are valuable input for plot-scale LST estimation.
 ...
 The estimation of plot-scale $\epsilon$ by fitting observed $H$ and estimated $\Delta T$ without an intercept resulted in unrealistically low $\epsilon$ values at some sites and the fit of a model without intercept is statistically questionable \cite{eisenhauer2003regression}. 
...
%SJS: what did you mean by "in site specific apporach"? I replaced it by "above", as there is no headline "site specific approach". Please check throughout.
...
\begin{figure}[h!]
	...
	\end{subfigure}
	\setlength{\belowcaptionskip}{-3ex}
	\caption{$ H and \Delta T$ plots at Howard Springs with optimised emissivity. %SJS: Git, can you please check all your text and figure titles and captions for correct spelling of Howard Springs and all the other sites? Or get someone else to check this if you just don't see it?
	}
	\label{fig:hs_hdt}
\end{figure}
...
\begin{figure}[h!]
	...
	\end{overpic}
	\setlength{\belowcaptionskip}{-3ex}
	\caption{(a) Uncertainty in plot-scale $\epsilon$ using the short equation (Seq) or the long equation (Leq, with and without intercept). (b) Uncertainty in hourly $T_s -T_a$ using long equation with and without intercept. %SJS: Need to place the "a" label in a visible place.
		%SJS: Something is weird here. If the uncertainty in epsilon is smaller for Leq with intercept, how can the uncertainty in Tleq-Ta be greater? The intercept only matters for epsilon, not for Tleq, so this does not make sense!
	}
	\label{fig:unc_intrc}
\end{figure}
