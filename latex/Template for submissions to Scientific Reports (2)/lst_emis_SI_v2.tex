...
The effects of global change are reflected in land surface temperature (LST) anomalies and their interannual variability \cite{rowell2005scenario}. It controls the magnitude and variability of the surface energy balance (SEB) components and simultaneously gets modulated by the SEB partitioning\cite{mallick2015reintroducing,timmermans2007intercomparison}. LST contains imprints of surface moisture and is extremely sensitive to evaporative cooling, which makes it a preeminent variable for studying evaporation and surface-atmosphere exchange \cite{mallick2018critical,trebs2021role,kustas2009advances}. It directly affects the amount of emitted longwave radiation and influences the saturation vapor pressure at the surface that drives latent heat flux. Thus, the ecohydrological functioning and carbon-water coupling are largely controlled by the surface temperature of the soil-vegetation system \cite{still2021imaging}.
 The availability of an extensive network of eddy covariance measurements (FLUXNET) allows us to understand the interactions and feedbacks between the surface-atmosphere exchange processes such as evaporation, transpiration, and its control by the atmosphere and vegetation at the diurnal time scale. However, the unavailability of direct LST measurements at the same scale hinders a detailed understanding of the interactions and feedbacks between LST and surface-atmosphere exchange processes, which is of utmost importance to the climate modeling community. %SJS: A reference is needed here, but when I opened not_indexed_ref.bib or lstpaper.bib to look for one, I found very rudimentary and broken files with missing and duplicated bibtex keys. How does it even work? I usually use my whole zotero library in papers (after converting to .bib) and only at the end extract the used references from the .bbl file and paste them into the latex document. 
...
\end{equation}
Reflected shortwave in Eq. ({\ref{eq_Rn1}}) is expressed as $R_{sref} = \alpha R_{sdown}$, where $\alpha$ is the surface albedo. Considering Kirchhoff's law, whereby the emissivity of a surface equals its absorptivity, emissivity values below unity result in reflected longwave radiation, expressed as: %SJS: I referred to Kirchhoff's law in case readers might be surprised about this.
...
%SJS: I moved most of this paragraph into the first paragraph of the intro, as it explains why LST is important, not how to estimate it.
...
The resulting intercept values ranged from -24 to +258 W m$^{-2}$, i.e. up to 70\% of the maximum observed sensible heat flux at a site (e.g. at HS and TUM).  %SJS: I don't think we can say much more, as the alleged pattern between closed and open canopies does not work out, given the negative intercept at Yatir and Daly uncleared. Also AR is not really a closed canopy. Therefore, I commented out the rest.
 %for BR, a grassland, was 12\% of the maximum observed sensible heat flux ($H_{max}$) and for a mulga shrubland (AS) it was 20\% of $H_{max}$ (Fig. \ref{fig:2_mx_c}). The intercept values for HS and LF were very high (70\% of $H_{max}$) and for TUM is 7\% of $H_{max}$. The intercept value is lowest for a homogenous land cover like TUM and highest for heterogeneous canopy like HS, LF. 
 Note, however, that if we assume just a slight under-estimation of upwelling longwave radiation by  40 W m$^-{2}$ (approx 8\% of observed $R_{lup}$), the intercept at HS was reduced from 237 to 17  W m$^-{2}$ (Fig. \ref{fig:mxc_dis}a). In this study, we did not apply any energy balance closure scheme, as a Bowen ratio closure resulted in even greater intercept values (Fig. \ref{fig:mxc_dis}b). %SJS: You should not introduce new figures in the discussion, unless they are not part of the main results, in which case they belong in the SI. It might we worth mentioning what happened if you closed the energy balance through upwelling longwave. Actually, it seems that the Bowen ratio closure increased the R2 while increasing the intercept, so I wonder what would happen if we assumed a 40 W under-estimation of upwelling longwave and then did the Bowen ratio closure on top of this. You could also just add the 40 W and still show the energy balance closure in the plot, without actually closing it. 
...
\begin{overpic}[width=0.45\textwidth]{brook_mx_c} % 
  ...
  \put (18,57){\textbf{d}}
   ...
   \end{subfigure}
 \setlength{\belowcaptionskip}{-3ex}
\caption{Sensible heat flux as a function of surface-to-air temperature difference based on Eq. \ref{eq_H2} ($H = m (T_{s} - T_{a}) + c$). $\epsilon$ was fitted to minimise RMSE of a robust linear regression. The title of the plot contains site, year, month and the fitted $\epsilon$-value. The legend correspond to Fig. \ref{fig:HDT}. %and c is interpreted as the $H$ from the aerodynamic footprint which is not seen by the radiometer. %SJS: I removed the last bit, as this does not belong in the figure caption, rather in the discussion.
	The colour code indicates the degree of energy imbalance of each data point (i.e. $R_{net} - H - LE - G$). %SJS: please check if this is correct. 
	}
\label{fig:2_mx_c}
\end{figure}

\begin{figure}[h!]
	\begin{overpic}[width=0.45\textwidth]{enb_hs_40} % 
		\put (18,58){\textbf{a}}
	\end{overpic}
	\begin{overpic}[width=0.45\textwidth]{enb_closed_40} % ,grid,tics=10
		\put (18,58){\textbf{b}}
	\end{overpic}
	\setlength{\belowcaptionskip}{-3ex}
	\caption{Sensible heat flux as a function of surface-to-air temperature difference based on Eq. \ref{eq_H2} ($H = m (T_{s} - T_{a}) + c$). Same analysis and legends as in Fig. \ref{fig:2_mx_c}c), but (a) After adding 40 ($W m^{-2}$) to measured $R_{lup}$, and (b) after closing the energy imbalance using a Bowen ratio closure scheme. %SJS: Insights to be gained belong in the main text, so I removed them.
	}
	\label{fig:mxc_dis}
\end{figure}
...
Each of the observed input variables used for the estimation of plot-scale $\epsilon$ and LST has a certain level of measurement uncertainty associated with it. Here we present exemplary results for Alice Springs, which showed the highest correlation between plot-scale and landscape scale LST estimations (Table \ref{table:eps_comp}).%(Table \textbf{SI2}). %SJS: Why SI2? Where is actually the SI??
...
%ranged between 0.68 and 0.98. %SJS: The quantiles of the simulated values ranged between 0.68 and 0.98, but this is not the uncertainty. Please check if my interpretation is correct.
The short equation (Eq.\ref{eq_Tseq}, 'seq') resulted in a vary narrow range of $\epsilon$ values between 0.94 and 0.99 throughout the year, with very small uncertainty (around $\pm$0.01, black boxes in Fig. \ref{fig:eps_unc1}a). %SJS: please check
...
\caption{Uncertainty in plot-scale estimations of $\epsilon$ and surface-air temperature differences ($T_{s} - T_{a}$) at Alice Springs (AS), based on Eq. \ref{eq_H} (no intercept in $H(\Delta T)$). Monthly values of $\epsilon$ shown for 2017 and hourly $T_{s} - T_{a}$ for 15 August 2017. \textbf{(a)} Uncertainty in monthly plot-scale emissivity due to uncertainty in $H$, $R_{lup}$, $R_{ldw}$ and $T_{a}$, using Eq. \ref{eq_leq} ('leq', blue)  and Eq. \ref{eq_seq} ('seq', black). \textbf{(b)} Hourly uncertainty in $T_{s} - T_{a}$ based on Eq. \ref{eq_leq}, due to uncertainty in $R_{lup}$, $R_{ldw}$ and $T_{a}$ when landscape-scale emissivity is used ($\epsilon_{MODIS}$, orange) or due to uncertainty in $H$, $R_{lup}$, $R_{ldw}$ and $T_{a}$ when plot-scale emissivity is used ($\epsilon_{plot}$, blue). \textbf{(c)} Same as Panel b, but based on Eq. \ref{eq_seq}. %SJS I'm confused. I thought that for each perturbation, you calculated epsilon and then Ts-Ta based on the epsilon and perturbed fluxes. Can you confirm? %GT you are correct
	Uncertainty calculated based on perturbations in input variables, where each box represents the median (red horizontal bar), 25\% and 75\% quantiles (filled bars) and range (whiskers) of output variables. %SJS: Please verify if I got it right here. 
	%SJS: Why did you not present the values based on intercept as well?
}
\label{fig:eps_unc1}
\end{figure}
...
This is illustrated e.g. in Fig. \ref{fig:eps_unc1}, where estimation of plot-scale $\epsilon$ resulted in similar LST values between the short and long equations, but with vastly different $\epsilon$ values and much greater uncertainty in estimated $\epsilon$ using the long equation compared to the short equation. Considering that the short equation ignores an important component of longwave radiation, it must be concluded that in this case, it achieves seemingly the right results for the wrong reasons.  %SJS I tried to focus a bit more.
% The use of long equation for plot-scale $\epsilon$ estimation results in lower values for canopy with heterogeneous land cover. In order to obtain realistic $\epsilon$ for sparse canopy with heterogeneous landcover, acknowledging footprint mismatch between radiometric and aerodynamic measurements becomes important. %SJS: I don't think we can link it to heterogeneous cover, as we have no evidence for this. LF and HS are not the only sites with heterogeneous cover.
%Depending on the equation (Eq. \ref{eq_Tleq} or Eq. \ref{eq_Tseq}) used to estimate LST, a small approximation (error) in $\epsilon$ can lead to a large error in LST (\textbf{SI Fig. \ref{fig:LST sensitivity to emissivity}}). %SJS: Statement included above in different form. 
The reduced sensitivity of the long equation (\ref{eq_Tleq}) to $\epsilon$ is of advantage for plot-scale LST estimation, since plot-scale $\epsilon$ is usually unknown and therefore used as an approximate value \cite{mallick2018bridging}. %Also,  plot-scale $\epsilon$ estimation using the long equation (Eq. \ref{eq_Tleq}) produced clear $H(\Delta T)$ relationships for more months ($R^{2} > 0.5$, blue stars in Fig. \ref{fig:HDT}d) than if the short equation was used. %SJS: I commented this out, as it seems to be weighing up between the short and the long equation, but this is not needed, as the short equation is herewith discarded.

However, when using the long equation in conjunction with plot-scale $H$ measurements to estimate plot-scale $\epsilon$, we obtained unrealistically low $\epsilon$ values at some sites (e.g. HS and LF, Table \ref{table:eps_comp}) in comparison to previously reported  $\epsilon$ values %SJS: "in the range between x and y"
...
The intercept (i.e. $\Delta T \ne 0$ at $H=0$) could be caused by combining measurements coming from instruments (radiometer, eddy covariance system) with different footprints\cite{marcolla2018geometry}. %SJS: I re-focused on the intercept here.
The mismatch of  source areas becomes important if the surface underlying the instruments  has heterogeneous land cover. % (e.g. clumped vegetation at AS, TT, YA, AS, HS, LF). %SJS: How did you choose these sites? Is AR and DU not clumped as well? Unless we have a measure of clumpiness, we don't have a good basis for discrimination.
 ...
 % and resulted into realistic $\epsilon$ (Fig {\ref{fig:2_mx_c}a, b, c, d). %SJS: I don't follow here, so I commented it out.
...
\begin{figure}[h!]
	...
	\end{overpic}
	\setlength{\belowcaptionskip}{-3ex}
	\caption{Uncertainity in plot-scale $\epsilon$ using long equation with and without intercept and hourly $T_s -T_a$}
	\label{fig:unc_intrc}
\end{figure}
