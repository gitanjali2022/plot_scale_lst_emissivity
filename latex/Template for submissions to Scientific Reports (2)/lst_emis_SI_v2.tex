...
\begin{enumerate}
	...
	%GT:I propose we rewrite the RQ.2 "What is the best approach to obtain plot-scale epsilon using eddy covariance measurements
	%and what is the advantage of using plot-scale epsilon for plot-scale LST estimation?
	% I plan to rewrite as we saw if we correct the intercept by correcting upwelling longwave even with MODIS epsilon we can reduce the bias between MODIS and plot-scale LST and get reasonable H vs DT plots
...
In summary, our results reveal that the short equation (neglecting down-welling longwave radiation) leads to biased estimates of LST and hugely over-estimated sensitivity of LST to surface emissivity. Therefore, the use of Eq. (\ref{eq_Tseq}) is not recommended and should be replaced by Eq. (\ref{eq_leq}) if measurements of down-welling longwave radiation are available. The representation of by $H (\Delta T)$ linearfit with an intercept is robust approach for emissivity estimation and this representation has additional advantage as it tells us about the extent of conjunction between radiometric and aerodynamic measurements. The uncertainty in diurnal LST 
increase by using plot-scale epsilon derived using $H (\Delta T)$ linearfit with an intercept. The increase in intercept by closing energy balance suggests the energy balance closure scheme do not necessarily account for footprint mismatch measurement bias. However, correction in up-welling longwave and energy balance closure together results into reliable $H$ vs $\Delta T$ plots with negligible intercept. The proposed approach of representing $H(\Delta T)$  with an intercept implies the  concept of "footprint awareness" explicitly when using eddy covariance fluxes in combination with local radiometer measurements. Therefore, the proposed method has the potential to provide a more robust benchmark information for model calibration and validation at plot-scale. Overall, the implications of our study are of particular relevance for the research community interested in understanding diurnal and seasonal feedback in soil-vegetation systems based on fluxes, emissivity and LST estimated at the plot-scale.  
