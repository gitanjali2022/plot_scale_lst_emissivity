...
In summary, our results reveal that the short equation Eq. (\ref{eq_Tseq}) (neglecting down-welling longwave radiation) leads to biased estimates of LST and hugely over-estimated sensitivity of LST to surface emissivity. Therefore, the use of Eq. (\ref{eq_Tseq}) is not recommended and should be replaced by Eq. (\ref{eq_Tleq}) when down-welling longwave radiation measurements are available. The use of Eq. (\ref{eq_Tleq}) results into unrealistic $\epsilon$ when $H (\Delta T)$ linearfit without intercept is used. The realistic $\epsilon$ by using Eq. (\ref{eq_Tleq}) is only possible when we include an intercept in $H (\Delta T)$ linearfit, but the value of intercept were significantly high at some sites. However, assuming the intercept as a consequence of foot-print mismatch, the proposed correction in up-welling longwave and subsequent energy balance closure resulted in negligible intercept and consistent $H(\Delta T)$ plots. This approach also reduced the bias between plot-scale LST and MODIS LST. The value of intercept can also be used as a criteria to check the consistency of observed data (radiometric and aerodynamic measurements) before using them in conjunction, as the high value of intercept indicates inconsistency between observed sensible heat flux and surface-to-air temperature difference. Therefore, the proposed method ($H (\Delta T)$ with an intercept) has the potential to provide a more robust benchmark information for model evaluation and validation at plot-scale. Overall, the implications of our study are of particular relevance for the research community interested in understanding the diurnal and seasonal feedback in soil-vegetation systems based on fluxes, emissivity and LST estimated at the plot-scale.  
...
The data and code used for this study is available on renkulab \url{https://renkulab.io/projects/gitanjali.thakur/plot-scale-lst-emissivity}. Before final publication, static versions of these repositories will be uploaded to zenodo.org and receive a separate DOI
